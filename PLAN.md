...
- **2026-07-30 — T1 batch-debounce cut from 2500ms to 700ms.** The authoritative node-placement call (`process-session`) was waiting behind a flat 2.5s client-side batching window before it even started, unrelated to Gemini/T2 speed — added purely to bundle finalized utterances into fewer, cheaper calls. Cut to 700ms; `BATCH_MAX_SIZE` (4) still catches real rapid-fire bursts. Confirmed by the user in live testing: "It works!" See [FINDINGS.md §20](FINDINGS.md#20-t1-authoritative-placement-was-gated-behind-a-flat-2500ms-batching-delay-2026-07-30).
- **2026-07-30 — RLS security fix, round 2.** A fresh Base44 security scan flagged 11 entities as publicly readable. The checked-in entity files already had correct owner-scoped `read` RLS; the live app was just running a rolled-back pre-RLS deploy (`good-version`, same branch as Finding 15), which never had read RLS to begin with. Fixed with a plain `entities push` from `main` (schema/RLS only, never touches data); verified via anonymous `curl` that all 11 now return an empty result instead of every user's records. See [FINDINGS.md §19](FINDINGS.md#19-rls-security-fix-round-2--a-good-version-rollback-had-reopened-read-access-2026-07-30).
...
2. **`process-session`** — Tier 1, the authoritative per-utterance classification (real placement + edge-connection, not the Stage 2 rough guess). Live capture batches finalized utterances client-side (`BATCH_DEBOUNCE_MS = 700` / `BATCH_MAX_SIZE = 4`) into one call instead of one call per utterance; import/wrap-up mode batches up to `IMPORT_BATCH_SIZE = 12` at a time. `BATCH_DEBOUNCE_MS` was `2500` until 2026-07-30 — real-time feel from cutting it to 700ms was judged worth a modest bump in call volume (the common one-utterance-then-pause case now rarely batches at all; `BATCH_MAX_SIZE` still catches genuine rapid-fire bursts). See [FINDINGS.md §20](FINDINGS.md#20-t1-authoritative-placement-was-gated-behind-a-flat-2500ms-batching-delay-2026-07-30).
