...
- **2026-07-30 — RLS security fix, round 2.** A fresh Base44 security scan flagged 11 entities as publicly readable. The checked-in entity files already had correct owner-scoped `read` RLS — the live deployment had silently drifted away from them a second time (see Finding 15 for the first). Fixed with a plain `entities push` (schema/RLS only, never touches data); verified via anonymous `curl` that all 11 now return an empty result instead of every user's records. Root cause of the drift itself wasn't pinned down. See [FINDINGS.md §19](FINDINGS.md#19-rls-security-fix-round-2--deployed-read-access-had-drifted-public-again-2026-07-30).
