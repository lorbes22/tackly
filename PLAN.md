...
- **Node detail panel** — slides in on click: summary, linked transcript excerpts, related nodes in the same session, resolve/done controls for Question/Risk/Action, and a delete/hide control. Notes are surfaced directly on the card face, not buried inside the panel below the transcript: a small "n notes" badge (only shown when at least one note exists) sits visibly on the node itself, and clicking that badge specifically — or the node as a whole — opens straight to viewing/adding notes, not a full panel scroll to find them. Deleting a node only hides it from the board — it doesn't touch the underlying utterance links or the node record itself, so the memory persists even though it's visually gone. Open question worth deciding during build: should a hidden node still be eligible for Tier 1 to re-attach new utterances to it? Default recommendation is no (the person deliberately hid it), but confirm rather than assume.
