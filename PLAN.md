...
- **Node detail panel** — slides in on click: summary, linked transcript excerpts, related nodes in the same session, resolve/done controls for Question/Risk/Action, and a delete/hide control. Notes UX (revised this pass): the old always-a-badge-when-count>0 top-right pill is gone — instead the bottom of every card has a small hover-revealed row. If the node already has notes, "n notes" stays visible there whether hovering or not; "+ Add note" only reveals on hover (or is the only thing shown, if there are no notes yet) and jumps straight to the panel's notes section, focused on the input — so adding the *first* note no longer requires opening the panel and scrolling to find where to type, which was previously only reachable once a note already existed. Deleting a node only hides it from the board — it doesn't touch the underlying utterance links or the node record itself, so the memory persists even though it's visually gone. Open question worth deciding during build: should a hidden node still be eligible for Tier 1 to re-attach new utterances to it? Default recommendation is no (the person deliberately hid it), but confirm rather than assume.
- **Search** — keyword search across a user's past sessions and nodes
- **Settings** — profile, plan/billing. The Plan section now shows real data (`UsageBadge`, `variant="detailed"`) instead of a static "you're on the free plan" sentence — plan name, minutes used/limit or "Unlimited", a progress bar, and a note when nearing the cap.
